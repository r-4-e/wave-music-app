...
function onDeviceReady() {
  ...
  // Dynamic runtime permission check for Android 13, 15 (Vanilla Ice Cream), and 16 (Baklava)
  if (window.cordova && cordova.plugins && cordova.plugins.permissions) {
    ...
    permissions.checkPermission(POST_NOTIFICATIONS, (status) => {
      if (!status.hasPermission) {
        permissions.requestPermission(POST_NOTIFICATIONS, 
          () => {
            console.log('[cordova] Notification permission granted.');
            initMusicControls();
          },
          () => {
            console.warn('[cordova] Notification permission denied. Media controls won\'t show.');
            initMusicControls();
          }
        );
      } else {
    ...
    // Immediate activation fallback for Android 12 and under
    initMusicControls();
  }
}
